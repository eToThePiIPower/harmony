defmodule Harmony.AccountTest do
  use Harmony.DataCase

  alias Harmony.Account

  import Harmony.AccountFixtures
  alias Harmony.Account.{User, UserToken}
  ...
  describe "apply_user_email/3" do
    ...
    test "requires email to change", %{user: user} do
      ...
      # Now try with the email explicitly set to the old one
  ...
  describe "update_user_email/2" do
    ...
    test "does not update email if token expired", %{user: user, token: token} do
      # Expiration is one week (7 days) ago, so set insertion time to one second before that
      week_ago = NaiveDateTime.utc_now |> NaiveDateTime.add(-7 * 24 * 3600 - 1)
      {1, nil} = Repo.update_all(UserToken, set: [inserted_at: week_ago])
      assert Account.update_user_email(user, token) == :error
      assert Repo.get!(User, user.id).email == user.email
      assert Repo.get_by(UserToken, user_id: user.id)
    end

    test "does update email if token is old but not expired", %{user: user, token: token} do
      # Expiration is a week ago, so set insertion time to one second after that
      week_ago = NaiveDateTime.utc_now |> NaiveDateTime.add(-7 * 24 * 3600 + 1)
      {1, nil} = Repo.update_all(UserToken, set: [inserted_at: week_ago])
      assert Account.update_user_email(user, token) == :ok
    end
  ...
  describe "get_user_by_session_token/1" do
    ...
    test "does not return user for expired token", %{token: token} do
      # Expiration is two months (60 days) ago, so set insertion time to one second before that
      too_old = NaiveDateTime.utc_now |> NaiveDateTime.add(-60 * 24 * 3600 - 1)
      {1, nil} = Repo.update_all(UserToken, set: [inserted_at: too_old])
  ...
  describe "confirm_user/1" do
    ...
    test "does not confirm email if token expired", %{user: user, token: token} do
      # Expiration is one week (7 days) ago, so set insertion time to one second before that
      too_old = NaiveDateTime.utc_now |> NaiveDateTime.add(-7 * 24 * 3600 - 1)
      {1, nil} = Repo.update_all(UserToken, set: [inserted_at: too_old])
